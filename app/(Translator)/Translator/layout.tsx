...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const viewAllNotifications = () => {
    setIsNotificationOpen(false);
    router.push('/Translator/Notifications');
  };
              ...
              {isNotificationOpen && (
                <div className="absolute right-0 mt-2 w-80 max-h-[70vh] overflow-auto bg-white rounded-xl shadow-lg border border-gray-100 z-50">
                  <div className="p-3 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <Link href="/Translator/Notifications" className="text-sm text-[#ff4e00] hover:underline">
                      View All
                    </Link>
                  </div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                ...
                <div className="hidden md:block text-left">
                  <p className="text-sm font-medium text-gray-700">{user?.name || "Translator"}</p>
                  <p className="text-xs text-gray-500">{user?.role || "Translator"}</p>
                </div>
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 p-2 bg-white rounded-xl shadow-lg border border-gray-100 z-50">
                  <div className="flex items-center gap-3 p-2 mb-1">
                    ...
                    <div>
                      <p className="text-sm font-medium">{user?.name || "Translator"}</p>
      ...
      <div className="flex flex-grow pt-16">
        <div className={`fixed left-0 top-0 h-full flex-shrink-0 z-30 transition-all duration-300 ${
          ...
          <div className={`${isMobile && 'relative'}`}>
            <TranslatorSidebar open={sidebarOpen} />

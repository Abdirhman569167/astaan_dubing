...
export default function AssignedTasksPage() {
  ...
  const fetchTasks = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_TASK_SERVICE_URL}/api/task-assignment/userAssignments/${user?.id}`);
  ...
  const updateTaskStatus = async (taskId: number, newStatus: string) => {
    setIsUpdating(true);
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_TASK_SERVICE_URL}/api/task-assignment/task_status_update/${taskId}`, {
        method: 'PUT',
        headers: {
  ...
  const handleSubmitTask = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_TASK_SERVICE_URL}/api/task-assignment/submitTask/${selectedTask.id}`, {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredTasks.map((task) => (
            <div
              key={task.id}
              className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-300 overflow-hidden cursor-pointer"
              onClick={() => {
                ...
                <div className="space-y-5 mb-6">
                  ...
                  {parseFileUrls(task.file_url).length > 0 && (
                    <div className="flex items-center gap-2 text-sm text-gray-500">
                      ...
                      <p>{parseFileUrls(task.file_url).length} attachment(s)</p>
                    </div>
                  )}
                </div>
              </div>
